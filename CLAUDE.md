...
slnx ソリューション (`SampleWpfMaf.slnx`) に 3 つのプロジェクトが共存：

| プロジェクト | 役割 |
|---|---|
| `src/SampleWpfMaf.Core/` | 共通クラスライブラリ。`AgentFactory`、`Tools/GuiTools`、`Settings/LlmSettings`、`Settings/AgentSettings` を提供 |
| `src/SampleWpfMaf/`      | テキストチャット版 (WinExe)。Core を参照 |
| `src/SampleWpfMafStt/`   | 上記 + マイク入力 (Sherpa-onnx / Moonshine ja + Silero VAD)。Core を参照 |

MAF 接続と GUI 操作 Tool は **`SampleWpfMaf.Core` に集約**されている。アプリ側の `AppSettings` は Core の `LlmSettings`/`AgentSettings` を合成し、STT 版だけ `SttSettings`/`AudioSettings` を追加する形。
...
task build              # dotnet build SampleWpfMaf.slnx
task run                # テキスト版 (SampleWpfMaf)
task run-stt            # 音声版 (SampleWpfMafStt) - 要 download-models
...
`models/` は gitignore 対象。`SampleWpfMafStt` のモデルパス解決 (`MoonshineRecognizer.ResolvePath`) は `AppContext.BaseDirectory` → 5 階層上のリポジトリルートの順で探すので、開発時 (`dotnet run`) もリリース時 (`bin/.../net10.0-windows/`) も同じ `appsettings.json` で動く。

dotnet 単体で動かしたい場合：

```sh
dotnet build SampleWpfMaf.slnx
dotnet run --project src/SampleWpfMaf/SampleWpfMaf.csproj
dotnet run --project src/SampleWpfMafStt/SampleWpfMafStt.csproj
```
...
## 参考: Tool 一覧 (`SampleWpfMaf.Core.Tools.GuiTools`)

LLM から呼べる Tool は両プロジェクト共通 (`src/SampleWpfMaf.Core/Tools/GuiTools.cs`)。追加時は両プロジェクトの `MainWindow.xaml.cs` の `aiTools` リストにも `AIFunctionFactory.Create` で登録すること。
