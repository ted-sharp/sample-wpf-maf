...
slnx ソリューション (`WpfMafSample.slnx`) に 3 つのプロジェクトが共存：

| プロジェクト | 役割 |
|---|---|
| `src/WpfMafSample.Core/` | 共通クラスライブラリ。`AgentFactory`、`Tools/GuiTools`、`Settings/LlmSettings`、`Settings/AgentSettings` を提供 |
| `src/WpfMafSample/`      | テキストチャット版 (WinExe)。Core を参照 |
| `src/WpfMafSampleStt/`   | 上記 + マイク入力 (Sherpa-onnx / Moonshine ja + Silero VAD)。Core を参照 |

MAF 接続と GUI 操作 Tool は **`WpfMafSample.Core` に集約**されている。アプリ側の `AppSettings` は Core の `LlmSettings`/`AgentSettings` を合成し、STT 版だけ `SttSettings`/`AudioSettings` を追加する形。
...
## 参考: Tool 一覧 (`WpfMafSample.Core.Tools.GuiTools`)

LLM から呼べる Tool は両プロジェクト共通 (`src/WpfMafSample.Core/Tools/GuiTools.cs`)。追加時は両プロジェクトの `MainWindow.xaml.cs` の `aiTools` リストにも `AIFunctionFactory.Create` で登録すること。
