...
├── .gitignore
├── .gitattributes
├── SampleWpfMaf.slnx            # slnx 形式 (XML ベースのソリューション)
└── src/
    └── SampleWpfMaf/
        ├── SampleWpfMaf.csproj  # TargetFramework: net10.0-windows
        ├── App.xaml
...
```xml
<Solution>
  <Project Path="src/SampleWpfMaf/SampleWpfMaf.csproj" />
</Solution>
```
...
# 第二プロジェクト: `SampleWpfMafStt`（リアルタイム音声入力サンプル）

## A. 概要

第一プロジェクト (`SampleWpfMaf`) と **同じ slnx ソリューションに追加**する、リアルタイム STT 対応版。
...
```
sample-wpf-maf/
├── SampleWpfMaf.slnx              # 既存（両プロジェクトを保持）
├── Taskfile.yml                   # ★追加: task init などのタスク定義
├── src/
│   ├── SampleWpfMaf/              # 既存
│   └── SampleWpfMafStt/           # ★追加
│       ├── SampleWpfMafStt.csproj
...
| `task run-stt`      | `dotnet run --project src/SampleWpfMafStt` |
| `task run`          | `dotnet run --project src/SampleWpfMaf` |
