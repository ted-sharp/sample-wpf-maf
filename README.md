...
| プロジェクト | 入力 | 説明 |
|---|---|---|
| `SampleWpfMaf`    | キーボード | テキストチャット → エージェント → GUI 操作 Tool |
| `SampleWpfMafStt` | マイク (PTT) | Sherpa-onnx (Moonshine ja) + Silero VAD で日本語音声 → エージェント |
...
| `task run`                           | テキスト版 (`SampleWpfMaf`) を実行 |
| `task run-stt`                       | 音声版 (`SampleWpfMafStt`) を実行 |
...
音声版 (`SampleWpfMafStt/appsettings.json`) には追加で `Stt` / `Audio` セクションがある。VAD オフ (`UseVad: false`)、デバッグ WAV 保存 (`SaveDebugWav: true` で `<bin>/debug_recordings/` に出力)、partial の更新間隔 (`PartialIntervalMs`) などをチューニングできる。詳細は `SPEC.md` 第 E 章。
...
├── SampleWpfMaf.slnx                    # slnx 形式ソリューション
